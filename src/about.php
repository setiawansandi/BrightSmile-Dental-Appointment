<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Home</title>
    <link rel="stylesheet" href="css/about.css" />
    <link rel="stylesheet" href="css/root.css" />
    <link
      href="https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@400;700&display=swap"
      rel="stylesheet"
    />
    <link
      href="https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700&display=swap"
      rel="stylesheet"
    />
  </head>

  <body>
    <?php require_once __DIR__ . '/components/navbar.php'; ?>

    <section class="about-hero">
      <img
        src="..\src\assets\images\about-hero.png"
        alt="Clinic"
        class="about-hero__img"
      />
      <div class="about-hero__caption">Your Smile, Our Priority</div>
    </section>

    <!-- About: content -->
    <section class="about">
      <div class="general about__grid">
        <h3 class="about__title">About <span class="highlight">Us</span></h3>

        <div class="about__text">
          <p>
            BrightSmile Dental Clinic is a leading Singaporean dental practice
            with a core mission to deliver quality and affordable dental care to
            the entire community.
          </p>
          <p>
            With many years of treating our patients like family, our team at
            BrightSmile Dental Clinic continuously strives to provide excellent
            customer experiences, taking the time to understand the unique needs
            of each patient. This commitment is supported by continuous training
            and education for our staff and clinicians to maintain the highest
            standards of dental care.
          </p>
          <p>
            Our dental team comes from diverse backgrounds and speaks multiple
            languages, including English, Mandarin, and Malay, ensuring that we
            can accommodate your needs and make every visit to the dentist a
            pleasant and memorable experience.
          </p>
        </div>

        <div class="about__brand">
          <img
            src="assets/icons/logo.svg"
            alt="BrightSmile logo"
            class="about__logo"
          />
          <div class="brand-name">BrightSmile</div>
        </div>
      </div>
    </section>

    <section class="contact general">
      <div class="contact-intro">
        <h2 class="contact-title">
          Get In <span class="highlight">Touch</span>
        </h2>
        <p class="contact-sub">
          We'd love to hear from you! Whether you have questions, need support,
          or want to learn more about our services, our team is here to help
        </p>
      </div>
      <div class="contact-info">
        <!-- Card 1 -->
        <div class="contact-item">
          <div class="icon-badge">
            <img
              src="../src/assets/icons/location-pin.svg"
              alt="Location icon"
            />
          </div>
          <div class="contact-meta">
            <div class="meta-title">Our Address</div>
            <div class="meta-text">
              Blk 18 Jln Membina,<br />#01-05, S164018
            </div>
          </div>
        </div>

        <!-- Card 2 -->
        <div class="contact-item">
          <div class="icon-badge">
            <img src="../src/assets/icons/phone-rounded.svg" alt="Phone icon" />
          </div>
          <div class="contact-meta">
            <div class="meta-title">Our Number</div>
            <div class="meta-text">
              <a href="tel:+6561235678">+65 6123 5678</a>
            </div>
          </div>
        </div>

        <!-- Card 3 -->
        <div class="contact-item">
          <div class="icon-badge">
            <img src="../src/assets/icons/email.svg" alt="Mail icon" />
          </div>
          <div class="contact-meta">
            <div class="meta-title">Our Email</div>
            <div class="meta-text">
              <a href="mailto:info@brightsmile.com">info@brightsmile.com</a>
            </div>
          </div>
        </div>
      </div>
    </section>

    <section>
      <div class="contact__map" aria-label="Map to our clinic">
        <iframe
          title="Bright Smile Dental Surgery (Tiong Bahru) location"
          loading="lazy"
          allowfullscreen
          referrerpolicy="no-referrer-when-downgrade"
          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2713.9050730170384!2d103.6801892161104!3d1.3445423607906486!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31da0f750415f4a3%3A0xc981943da10e306e!2sNanyang%20Auditorium!5e0!3m2!1sen!2ssg!4v1760380046335!5m2!1sen!2ssg"
        >
        </iframe>
      </div>
    </section>

    <!-- Footer -->
    <footer class="footer">
      <div class="footer-content">
        <!-- Left Column -->
        <div class="footer-left">
          <div class="footer-header">
            <img
              src="assets/icons/logo.svg"
              alt="Tooth Icon"
              class="footer-logo"
            />
            <div class="brand-name">BrightSmile</div>
          </div>
          <p>
            BrightSmile offers gentle, modern dental care with clear guidance
            and advanced technology. Comfortable visits, from checkups to
            cosmetic treatments.
          </p>
          <p class="copyright">© 2025 BrightSmile. All Rights Reserved.</p>
        </div>

        <!-- Right Column -->
        <div class="footer-right">
          <div class="footer-header">
            <div class="links-title">Quick Links</div>
          </div>
          <ul>
            <li><a href="">Home</a></li>
            <li><a href="">Services</a></li>
            <li><a href="">Clinics</a></li>
            <li><a href="">About</a></li>
            <li><a href="">More</a></li>
          </ul>
        </div>
      </div>
    </footer>
  </body>
</html>
